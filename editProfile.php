...
<?php
    ...
    if(isset($_SESSION['username'])){
        ...
        function startsWith ($string, $startString){
            ...
            $tglLahir = date('Y-m-d', strtotime($row['tanggalLahir']));
            $today = date('Y-m-d');
            $nik = $row['NIK'];
        ...
        if(isset($_POST['submit'])){
            ...
            $str_query ="select * from customer where NIK = '".$_POST['nik']."' except select * from customer where username = '".$_SESSION['username']."'";
            $query = mysqli_query($connection, $str_query);
            ...
            $str_query ="select * from customer where username = '".$_POST['username']."' except select * from customer where username = '".$_SESSION['username']."'";
            $query = mysqli_query($connection, $str_query);
        ...
        if(count($error)==0){
            ...
            if($_FILES['fotoProfil']['size']>0){
                ...
                while($row = mysqli_fetch_array($query)){
                    if($filename == $row['fotoProfil']){
                        $filename = "(1)".$row['fotoProfil'];
                        $fotoProfil = $filename;
                    }
                }
                
                $terupload = move_uploaded_file($tmpName, $dirUpload.$filename);
                unlink($dirUpload."/".$oldFileName);
            }

            $str_query= "update customer set namaDepan='".$_POST['namaDepan'].
            "', namaTengah='".$_POST['namaTengah'].
            "', namaBelakang='".$_POST['namaBelakang'].
            "', tempatLahir='".$_POST['tempatLahir'].
           ...
           "' where username = '".$_SESSION['username']."'";
            ...
            if($query){
                echo ("<script LANGUAGE='JavaScript'> window.alert('Update Berhasil'); window.location.href='./profile.php';</script>");
            }else{
                echo ("<script LANGUAGE='JavaScript'> window.alert('Update Gagal');</script>");
            }
            ...
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="field">
                        <div class="foto-container">
                            <div class="foto">
                                <label for="fotoProfil">Ganti Foto</label>
                ...
                <div class="row">
                    <div class="field">
                        ...
                        <input type="text" name="username" id="" readonly value="<?php echo $username ?>">
                    </div>
                    <div class="field">
                ...
                <div class="button">
                    ...
                    <input type="submit" value="Update" id="register" class="submit" name ="submit" onclick ="return confirm('Apakah anda yakin ingin melakukan Update Data ?')">
                </div>
            </form>
        </div>
    </div>
